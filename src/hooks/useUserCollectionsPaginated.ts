/**
 * User Collections Paginated Hook
 *
 * 특정 사용자가 만든 컬렉션 목록 조회를 위한 paginated hook
...
interface UseUserCollectionsPaginatedReturn {
  // 데이터
 ...
 *   loadMore,
 *   hasMore
 * } = useUserCollectionsPaginated('123', 'infinite');
 * ```
 */
export function useUserCollectionsPaginated(
  userId: string | undefined,
  mode: ScrollMode = 'infinite'
): UseUserCollectionsPaginatedReturn {
  const {
    page,
    limit,
    sortBy,
  ...
  const fetchCollections = useCallback(async (append: boolean = false) => {
    ...
    try {
      ...
      console.log('🔍 Fetching user collections:', { userId, page, limit, sortBy, append });

      // API 호출: GET /users/{userId}/collections
  ...
  const refresh = useCallback(() => {
    console.log('🔄 Refreshing user collections');
    setCollections([]);
    setPage(1);
