/**
 * Liked Collections Paginated Hook
 *
 * 좋아요한 컬렉션 목록 조회를 위한 paginated hook
...
interface UseLikedCollectionsPaginatedReturn {
  // 데이터
 ...
 * ```
 */
export function useLikedCollectionsPaginated(
  mode: ScrollMode = 'infinite'
): UseLikedCollectionsPaginatedReturn {
  const {
    page,
    limit,
    sortBy,
  ...
  const fetchCollections = useCallback(async (append: boolean = false) => {
    try {
      ...
      console.log('🔍 Fetching liked collections:', { userId, page, limit, sortBy, append });

      // API 호출: GET /users/{userId}/collections/liked
      const response = await apiService.users.getMyCollections_4(
        userId,
        sortBy,
        page,
        limit
      ...
      console.log('✅ Liked collections fetched:', {
  ...
  const refresh = useCallback(() => {
    console.log('🔄 Refreshing liked collections');
    setCollections([]);
    setPage(1);
