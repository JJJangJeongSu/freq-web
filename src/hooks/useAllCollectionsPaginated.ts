/**
 * All Collections Paginated Hook
 *
 * 공개된 컬렉션 전체 목록 조회를 위한 paginated hook
...
export function useAllCollectionsPaginated(
  ...
  const fetchCollections = useCallback(async (append: boolean = false) => {
    try {
      ...
      console.log('🔍 Fetching collections:', { page, limit, sortBy, append });

      // API 호출: GET /collections/all
